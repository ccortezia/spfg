...
TEST(build_dp, test_spfg_dp_remove_should_keep_data_integrity)
{
...

    {
        ...
        TEST_ASSERT_EQUAL(SPFG_ERROR_NO, spfg_fn_create(&runtime, SPFG_FN_AND_BOOL_BOOL_RET_BOOL, 1, in_dps, 2, out_dps, 1, "fn1", &fn0));
    }

    {
        ...
        TEST_ASSERT_EQUAL(SPFG_ERROR_NO, spfg_dp_create(&runtime, SPFG_DP_BOOL, "dp5", &dp5));
        spfg_dp_id_t in_dps[] = {dp3, dp4};
        spfg_dp_id_t out_dps[] = {dp5};
        TEST_ASSERT_EQUAL(SPFG_ERROR_NO, spfg_fn_create(&runtime, SPFG_FN_AND_BOOL_BOOL_RET_BOOL, 1, in_dps, 2, out_dps, 1, "fn2", &fn1));
    }

    TEST_ASSERT_EQUAL(SPFG_ERROR_INVALID_DP_ID, spfg_dp_remove(&runtime, -1));
    TEST_ASSERT_EQUAL(SPFG_ERROR_FN_INTEGRITY, spfg_dp_remove(&runtime, dp0));
    TEST_ASSERT_EQUAL(SPFG_ERROR_FN_INTEGRITY, spfg_dp_remove(&runtime, dp2));
    TEST_ASSERT_EQUAL(SPFG_ERROR_NO, spfg_fn_remove(&runtime, fn0));
    TEST_ASSERT_EQUAL(SPFG_ERROR_NO, spfg_dp_remove(&runtime, dp0));
    TEST_ASSERT_EQUAL(SPFG_ERROR_NO, spfg_dp_remove(&runtime, dp1));
    TEST_ASSERT_EQUAL(SPFG_ERROR_NO, spfg_dp_remove(&runtime, dp2));
}
